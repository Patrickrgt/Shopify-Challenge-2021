...
class Home extends React.Component {
  ...
  constructor(props) {
    ...
    const { cookies } = props;
    this.state = {
      apodLiked: cookies.get("apodLiked") || [],
      //   API Key
  ...
  async APODImage() {
    ...
    const { copyright } = this.state.apodQuery;
    const { likedDates } = {
      likedDates: this.state.apodLiked.map((a) => a.date),
    };
    const { dates } = { dates: this.state.apodQuery.map((a) => a.date) };

    console.log(this.state.apodQuery);
    console.log(dates);

    // cookies.set("apodQuery", , { path: "/" });
    console.log(dates);
    console.log(likedDates);

    console.log(dates.includes(likedDates[0]));
  }
  ...
  async like(e, i) {
    ...
    if (this.state.activeTab === "APOD") {
      await this.setState(
        ...
        () => {
          const { date } = this.state.apodLiked;
          cookies.set("apodLiked", this.state.apodLiked, { path: "/" });
          console.log(cookies);
        }
      );
    }
